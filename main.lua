-- all this was really hard to make so please like and sub to: @bofloki
...
do
    local Window = Fluent:CreateWindow({
        Title = string.format("%s <b><i>%s</i></b>", string.format(MonthlyLabels[os.date("*t").month], "Aimbot"), #Status > 0 and Status or "🔥FREE🔥"),
        SubTitle = "by: floki",
    ...
    Tabs.Aimbot:AddParagraph({
        Title = string.format("%s 🔥FREE🔥", string.format(MonthlyLabels[os.date("*t").month], "Aimbot")),
    ...
    AimbotSection:AddButton({
        ...
        Callback = function()
            ...
            AimPartDropdown:SetValues(Configuration.AimPartDropdownValues)
            Window:Dialog({
                Title = string.format(MonthlyLabels[os.date("*t").month], "Aimbot"),
    ...
    PremiumChecksSection:AddParagraph({
        Title = string.format("%s 💫PREMIUM💫", string.format(MonthlyLabels[os.date("*t").month], "Aimbot")),
    ...
    if UISettings.ShowWarnings then
        ...
        elseif ShowWarning then
            Window:Dialog({
                Title = "Warning",
                Content = string.format("Your Software does not support all the Features of %s 🔥FREE🔥!", string.format(MonthlyLabels[os.date("*t").month], "Aimbot")),
                Buttons = {
                    {
                        Title = "Confirm"
                    }
...
local function Notify(Message)
    if Fluent and typeof(Message) == "string" then
        Fluent:Notify({
            Title = string.format("%s ", string.format(MonthlyLabels[os.date("*t").month], "Aimbot")),
            Content = Message,
            SubContent = "by: floki",
            Duration = 1.5
        })
